...
data_frame <- as.data.frame(spark12)
filtered_data_frame <- data_frame[data_frame$adjusted_pvalue < 0.01,]

#filtro los genes que he seleccionado en mi spe original  
genes_to_keep <- rownames(filtered_data_frame)
spe_filtered <- spe1[genes_to_keep, ]
...
#la k es el numero de vencinos que considera 
k <- 22
set.seed(123)
...
pal <- c("magenta", "darkorange", "darkblue","green","yellow",'black','red')
